...
import { ArrowUpDown, ArrowUp, ArrowDown, Instagram, Activity } from 'lucide-react';
import { formatNumber, formatPercent } from '../lib/format.js';

const PLATFORM_FILTERS = [
  { key: 'all', label: 'Semua', icon: null },
  { key: 'instagram', label: 'IG', icon: Instagram },
  { key: 'tiktok', label: 'TT', icon: Activity }
];
        ...
        {PLATFORM_FILTERS.map((f) => {
          ...
          return (
            <button
              ...
              className={`chip transition-colors ${platform === f.key ? 'bg-accent-primary text-white' : 'bg-bg-tertiary text-text-secondary hover:text-text-primary'}`}
            >
              ...
              <tr key={a.slug} className="border-b border-border-subtle/50 hover:bg-bg-tertiary/50">
                ...
                <td className="py-3 px-4 text-text-secondary">
                  <span className="inline-flex items-center gap-1">
                    {a.platform === 'instagram' ? <Instagram className="w-3 h-3 text-pink-500" /> : <Activity className="w-3 h-3 text-cyan-500" />}
                ...
                <td className="py-3 px-4 text-right tabular-nums">
                  ...
                  }`}>{a.healthScore ?? 0} ({a.healthGrade ?? '—'})</span>
                </td>
              </tr>
            ))}
          </tbody>
