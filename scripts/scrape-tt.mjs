...
async function scrapeAccount(account, tokenPool) {
  ...
  console.log(`\n[TT] @${username} — starting (depth=${FULL_SCRAPE_DEPTH})`);
  ...
  console.log(`[TT] @${username} — DONE. ${deduped.length} posts → ${path.basename(outPath)} (${Math.round((Date.now() - startTime) / 1000)}s)`);
  return out;
}
